...
int main(void) {
  ...
  retval = robot.MoveJ(&start_jointpose, &start_descpose, 1, 0, 50, 50, 50,
                       &ex_axis_pose, 0, 0, &offset_pose);
  if (retval != 0) {
  ...
  retval = robot.MoveL(&end_jointpose, &end_descpose, 1, 0, 30, 50, 50, 0,
                       &ex_axis_pose, 0, 0, &offset_pose);
  if (retval != 0) {
