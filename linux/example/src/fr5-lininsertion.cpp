...
int main(void) {
  ...
  uint8_t ILC_sign = 0;  // ILC control start stop state, 0- stop, 1- training,
                         // 2- real operation
  ...
  float force_goal =
      20.0;             // Force or torque threshold (0 to 100), unit: N or Nm
  ...
  float forceInsertion =
      1.0;           // Force or torque threshold (0 to 100), in N or Nm
  int angleMax = 45; // Maximum rotation Angle, unit: °
  uint8_t orn = 1;   // Direction of force，1-fz,2-mz
  float angAccmax =
      0.0; // Maximum angular acceleration of rotation, unit: °/s^2, not in use
  uint8_t rotorn = 1; // Rotation direction, 1- clockwise, 2- counterclockwise

  uint8_t select1[6] = {
      0, 0, 1, 1,
      1, 0}; // Six degrees of freedom options [fx,fy,fz,mx,my,mz], 0- does not
             // work, 1- works
  ft.fz = -10.0;
  robot.FT_Control(status, sensor_num, select1, &ft, gain, adj_sign, ILC_sign,
                   max_dis, max_ang);
  ...
  uint8_t select2[6] = {
      1, 1, 1, 0,
      0, 0}; // Six degrees of freedom options [fx,fy,fz,mx,my,mz], 0- does not
             // work, 1- works
  gain[0] = 0.00005;
  ft.fz = -30.0;
  status = 1;
  robot.FT_Control(status, sensor_num, select2, &ft, gain, adj_sign, ILC_sign,
                   max_dis, max_ang);
  ...
  gain[0] = 0.0001;
  status = 1;
  robot.FT_Control(status, sensor_num, select3, &ft, gain, adj_sign, ILC_sign,
                   max_dis, max_ang);
  robot.FT_RotInsertion(rcs, angVelRot, forceInsertion, angleMax, orn,
                        angAccmax, rotorn);
  status = 0;
  robot.FT_Control(status, sensor_num, select3, &ft, gain, adj_sign, ILC_sign,
                   max_dis, max_ang);
  ...
  ft.fz = -30.0;
  status = 1;
  robot.FT_Control(status, sensor_num, select4, &ft, gain, adj_sign, ILC_sign,
                   max_dis, max_ang);
  robot.FT_LinInsertion(rcs, force_goal, lin_v, lin_a, disMax, linorn);
  status = 0;
  robot.FT_Control(status, sensor_num, select4, &ft, gain, adj_sign, ILC_sign,
