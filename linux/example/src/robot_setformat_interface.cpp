...
int main(void) {
  ...
  ret = robot.NewSplinePoint(&j1, &desc_pos1, tool, user, vel, acc, ovl, blendR,
                             0);
  printf("NewSplinePoint ret = %d\n", ret);

  ret = robot.NewSplinePoint(&j2, &desc_pos2, tool, user, vel, acc, ovl, blendR,
                             0);
  printf("NewSplinePoint ret = %d\n", ret);

  ret = robot.NewSplinePoint(&j3, &desc_pos3, tool, user, vel, acc, ovl, blendR,
                             0);
  printf("NewSplinePoint ret = %d\n", ret);

  ret = robot.NewSplinePoint(&j4, &desc_pos4, tool, user, vel, acc, ovl, blendR,
                             1);
  printf("NewSplinePoint ret = %d\n", ret);
